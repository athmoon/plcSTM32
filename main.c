...
static __IO uint32_t input,output;
...
SEQ_Ciclo  StatoGiorno;
SEQ_Cometa StatoCometa;

static  uint16_t T_1,T_2,T_3,T_4,T_5,T_6,T_7,T_8;    // tempo delle rampe  in secondi

uint8_t sl1,sl2,sl3,sl4;  // setpoint
uint8_t sl_min,sl_max;

// *****************  **************  ***********************


static __IO bool data_recived;
static __IO unsigned char scandata,n_byte;
...
//         8 -> input PU/PD ( PU ODR -> 1 ); B -> Output push-pull alternate func   
 ...
 GPIOA->CRL =    0xB4B40080;
 GPIOA->CRH =    0x4443444B;
 GPIOA->ODR =    0x00001000; 
 GPIOB->CRL =    0xBB488443;
 GPIOB->CRH =    0x83333388;
 GPIOC->CRL =    0x77000000;
 GPIOC->CRH =    0x44438777;
 ...
 // configurazione interrupt su PA1  per sincronismo di rete
...
//***************  inizializzazioni  ****************
 ...
 start_T1=FALSE;
 start_T3=FALSE;
 val_tim1=val_tim2=val_tim3=val_tim4=0;
 DESELECT(); 
 search=0;
 found=0;
 ccyc=0;
...
// ram_data = malloc (sizeRam);
 ...
 tim4=100;  // per calcolo rampa luci
...
*/  
  ...
  if ( SD_mount ==  0 ) {  // se scheda SD inizializzata correttamente
    seleziona_messaggio(0);  // seleziona le prime due righe ( 0 e 1 ) contenute nel file testi
    FLASH_UnlockBank1();
    ...
    FLASH_LockBank1();
    seleziona_messaggio(2);  //  // seleziona le righe  2 e 3  contenute nel file testi
    if ( pRiga[33] == 49 ) { // se l'ultimo carattere della riga 3 è 1 impostare data ed ora
        Pcalc=0;
...
// ************* inizializzazioni per luci    ************* 
 sl_min=20;
 sl_max=96;
 sl1=sl2=sl3=sl4 = sl_min;   //set luci minimo
 tempo_ciclo = 0;
 StatoGiorno = RampaGiorno;
 StatoCometa = offCometa;
 T_1 = 300;  //P[0];
 T_2 = 150;  //P[1];
 T_3 = 300;  //P[2];
 T_4 = 100;  //P[3];
 ...
 STM32F100PinSet( GPIOA , GPIO_Pin_12 , FALSE);  //  attiva le uscite sui fotoaccoppiatori
...
//     STM32F100PinToggle( GPIOC,GPIO_Pin_9 );   
    ...
    if ( (idisp < 35) || Disp_SPI_BSY ) 
     {                        //-> visualizza dati selezionati in ram
/*     
       if ( idisp == 0 ) 
       {
         pRiga=ram; 
    ...
    if ( (cifra != cifraP) && (!Disp_SPI_BSY)  && (idisp >= 35) ) 
     {	 //-> visualizzazione dei dati digitati sul display
       sequenza++;
       Display(cifra,posiz);      
     } 

    if ( req_ram_write )  
     {  // condizioni di scrittura dati su ram_data  	                                   
       sequenzaD++;
       if ( newSect2write ) 
        {
            d=ram_data;
    ...
    if ( dataDisp < 10  && !start_T2  ) 
     {     //-> imposta visualizzazione dati ciclica                                        
       start_T2=TRUE;
       switch (dataDisp)
       {
        case 1: { 
...
/*     case 8: {      
    ...
    if ( numFlash  != 0 )  
     {  // cancella la pagina flash  e riscrive i parametri
    ...
    if ( OnOff ) SequenzaLuci();    //  gestione luci
    
    if ( sel == 4 )                 // visualizza stato attuale sequenza luci
     {
       switch (StatoGiorno)
       {
          case RampaGiorno       : {display = 8;break;}
          case Giorno            : {display = 10;break;}
          case RampaTramonto     : {display = 12;break;}
          case Tramonto          : {display = 14;break;}         
          case RampaNotte        : {display = 16;break;}
          case Notte             : {display = 18;break;}
          case RampaGiornoNotte  : {display = 24;break;}
          default: ;
       } 
...
uint32_t tastierino(void)
{
  ...
  { 
   ...
   case 181: {                   // tasto " / " del tastierino
    ...
    case 220: {                    // tasto " D " della tastiera
	        sel=13;            // limite massimo accensione 
    ...
    case 227: {                    // tasto " A " della tastiera
	        sel=14;            // limite minimo accensione
	        valore=sl_min;
                display=28;
                posiz = 75;
		peso_tst=1;
  ...
  case 250: {                        // F1  durata rampa giorno
		sel=201;            
		valore = T_1;
                display = 8;
                posiz = 75;
  ...
  case 251: {                    // F3  durata rampa tramonto
		sel=203;            
		valore = T_3;
                display = 12;
                posiz = 75;
  ...
  case 243: {                    // F4  durata rampa tramonto
		sel=204;            
		valore = T_4;
                display = 14;
                posiz = 75;
  ...
  case 252: {                    // F5  durata rampa notte
		sel=205;            
		valore = T_5;
                display = 16;
                posiz = 75;
  ...
  case 124: {                    // F7  durata rampa cometa
		sel=207;            
		valore = T_7;
                display = 20;
                posiz = 75;
  ...
  if ( temp_tst == 10) 
   { 
    ...
    if ( sel == 151 )             // tasto "1" tastiera
     {
       sl1 = valore;   
       return valore;
     }
    if ( sel == 152 )             // tasto "2" tastiera
     {
       sl2 = valore;   
       return valore;
     }
    if ( sel == 153 )             // tasto "3" tastiera
     {
       sl3 = valore;   
       return valore;
     }
    if ( sel == 154 )             // tasto "4" tastiera
     {
       sl4 = valore;   
       return valore;
     }
     
    if ( sel == 201 )             // tasto "F1" tastiera
     {
       T_1 = valore;   
       return valore;
     }
    if ( sel == 202 )             // tasto "F2" tastiera
     {
       T_2 = valore;   
       return valore;
     }
    if ( sel == 203 )             // tasto "F3" tastiera
     {
       T_3 = valore;   
       return valore;
     }
    if ( sel == 204 )             // tasto "F4" tastiera
     {
       T_4 = valore;   
       return valore;
     }
    if ( sel == 205 )             // tasto "F5" tastiera
     {
       T_5 = valore;   
       return valore;
     }
    if ( sel == 206 )             // tasto "F6" tastiera
     {
       T_6 = valore;   
       return valore;
     }
    if ( sel == 207 )             // tasto "F7" tastiera
     {
       T_7 = valore;   
       return valore;
     }
    if ( sel == 208 )             // tasto "F8" tastiera
     {
       T_8 = valore;   
       return valore;
     }
...
void visualizza(void) // portando idisp = 0 visualizza i caratteri puntati da pRiga
{
  if ( idisp == 0 ) 
  { 
   FuncDisp(SetDDram , 0);
  ...
  if(val_tim3++  > tim3 )   // tim3 = 1000 per orologio
   {
    ...
    //tempo_ciclo++;     // contasecondi utilizzati in luci
    //tempo_cometa++;
    
   } 

  if(val_tim4++  > tim4 )   // tim4 = 100 per orologio
   {
...
void LeggiInput(void)

{

   STM32F100PinSet( GPIOC , GPIO_Pin_10 , FALSE);  //  congela gli ingressi nei registri de 4021  porta strobin = 0

   unsigned  int selettore=0x0001;
   volatile char i,j; 


   if ( GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_11) != 0 )  // leggi DATin
	  input |= selettore;
   else
	  input &= ~selettore;
   ...
   { 
     ...
     STM32F100PinSet( GPIOB , GPIO_Pin_7 , TRUE);   // set CKin
     for(j=0;j<10;j++) ;
     selettore <<= 1;
     if ( GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_11) != 0 )  // leggi DATin
	input |= selettore;
        ...
        input &= ~selettore;
     STM32F100PinSet( GPIOB , GPIO_Pin_7 , FALSE);  // res CKin

   }

 //  SetCkIn();
    STM32F100PinSet( GPIOC , GPIO_Pin_10 , TRUE); //  aggiorna gli ingressi nei registri de 4021  porta strobin = 1
...

{
   ...
   for (i=0;i<32;i++) 
   {
     if ( output & sel ) STM32F100PinSet( GPIOC , GPIO_Pin_8 , TRUE);   // set DATout
	   else          STM32F100PinSet( GPIOC , GPIO_Pin_8 , FALSE);  // res DATout
	 STM32F100PinSet( GPIOC , GPIO_Pin_6 , FALSE);  // res CKout
	 sel <<= 1;
	 for(j=0;j<10;j++) ;
	 STM32F100PinSet( GPIOC , GPIO_Pin_6 , TRUE);  // set CKout       

   }
   
   STM32F100PinSet( GPIOC , GPIO_Pin_7 , TRUE);  // trasferisce il buffer 4094 alle uscite
   for(j=0;j<10;j++) ;
   STM32F100PinSet( GPIOC , GPIO_Pin_7 , FALSE); // ( genera impulso su strobout )
}


//***  Gestione sequenza luci

void  SequenzaLuci(void) 
{
  ...
  switch (StatoGiorno)
   {
     case RampaGiorno :       { 
                                if ( tempo_ciclo < T_1 )
                                 {
                                  aux_sl= sl_min + ( sl_max - sl_min )*tempo_ciclo/T_1;                                 
                                  sl1=(uint8_t)trunc(aux_sl);
                                  if (sl1 > sl_max) sl1=sl_max;
                                 }
                                else
                                 {
                                  tempo_ciclo=0;
     ...
     case RampaTramonto :     { 
                                if ( tempo_ciclo < T_3 )
                                 {
                                  aux_sl= sl_min + ( sl_max - sl_min )*tempo_ciclo/T_3;                                 
                                  sl2=(uint8_t)trunc(aux_sl);
                                  if (sl2 > sl_max) sl2=sl_max;
                                 }
                                else
                                 {
                                  tempo_ciclo=0;
                                  StatoGiorno = Tramonto;
                                 } 
                                break;}
     ...
     case Tramonto :          { 
                                if ( tempo_ciclo > T_4 )
                                 {
                                  tempo_ciclo=0;
                                  StatoGiorno = RampaNotte;
                                 }                                 
                                break;}
                                
     case RampaNotte :        { 
                                if ( (tempo_ciclo < T_5) || (tempo_ciclo < T_1) || (tempo_ciclo < T_3) )
                                 {
                                  if ( tempo_ciclo <= T_1 )
                                   {
                                     aux_sl= sl_min + ( sl_max - sl_min )*(T_1 - tempo_ciclo)/T_1; 
                                     sl1=(uint8_t)trunc(aux_sl);
                                   }
                                   else
                                     sl1= sl_min;
                                  ...
                                  if ( tempo_ciclo <= T_3 )
                                   {
                                     aux_sl= sl_min + ( sl_max - sl_min )*(T_3 - tempo_ciclo)/T_3; 
                                     sl2=(uint8_t)trunc(aux_sl);
                                   } 
                                   else
                                     sl2= sl_min;
                                  if ( tempo_ciclo <= T_5 )
                                   {
                                    aux_sl= sl_min + ( sl_max - sl_min )*tempo_ciclo/T_5;                                 
                                    sl3=(uint8_t)trunc(aux_sl);
                                   }
                                   else
                                     sl3 = sl_max;
                                 }
     ...
     case Notte :             { 
                                if ( ( tempo_ciclo < T_6 ) || ( StatoCometa != CometaOff ))
                                {
                                  switch (StatoCometa)
                                  {
                                   case incRampaCometa  :    { 
                                     if ( tempo_cometa < T_7 )
                                      {
                                        aux_sl= sl_min + ( sl_max - sl_min )*tempo_cometa/T_7;                                 
                                        sl4=(uint8_t)trunc(aux_sl);
                                        if (sl4 > sl_max) sl4=sl_max;
                                      }
                                     else
                                      {
                                        tempo_cometa=0;
                                        sl4=sl_max;
                                        StatoCometa = CometaOn;
                                      }                                      
                                     break;}
                                   case CometaOn        :    {
                                     if ( tempo_cometa > T_8 )
                                      {
                                        tempo_cometa=0;
                                        StatoCometa = decRampaCometa;
                                      }                                                                          
                                     break;}
                                   case decRampaCometa  :    {
                                     if ( tempo_cometa < T_7 )
                                      {
                                        aux_sl= sl_min + ( sl_max - sl_min )*(T_7 - tempo_cometa)/T_7;                                 
                                        sl4=(uint8_t)trunc(aux_sl);
                                        if (sl4 < sl_min) sl4=sl_min;
                                      }
                                     else
                                      {
                                        tempo_cometa=0;
                                        sl4=sl_min;
                                        StatoCometa = CometaOff;
                                      }                                      
                                ...
                                else
                                {
                                   StatoCometa = offCometa;
                                   tempo_ciclo=0;
                                   StatoGiorno = RampaGiornoNotte;
                                }         
                                break;}
                                
     case RampaGiornoNotte :  { 
                                if ( ( tempo_ciclo < T_1 ) || ( tempo_ciclo < T_5 ) )
                                 {
                                   ...
                                   if ( tempo_ciclo <= T_5 )
                                   {
                                     aux_sl= sl_min + ( sl_max - sl_min )*(T_5 - tempo_ciclo)/T_5; 
                                     sl3=(uint8_t)trunc(aux_sl);
                                   }
                                   else
                                     sl3= sl_min;
